...
export default function EmailIntelligence({ theme }) {
  ...
  const handleAction = async (action, email, index) => {
    ...
    try {
      ...
      setRawResponse(response);
      setActionMessage(
        `${action === "calendar" ? "Calendar" : "Notion"} action submitted for “${email.subject || "Untitled"}”.`,
      );
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <section className="grid gap-4 md:grid-cols-3">
        {[
          {
            label: "Scanned Emails",
            value: payload.scannedCount || payload.emails.length,
          },
          ...
          <div
            key={stat.label}
            className={`rounded-[28px] border p-5 shadow-xl ${cardClass}`}
          >
      ...
      <div className="grid gap-6 xl:grid-cols-[1.35fr_0.85fr]">
        <ResultCard
          ...
          subtitle="AI classified emails ready for action."
          theme={theme}
        >
          <div className="space-y-4">
            {payload.emails.length === 0 ? (
              <div
                className={`rounded-3xl border border-dashed p-8 text-center text-sm text-slate-400 ${cardClass}`}
              >
                Click{" "}
                <span className="font-semibold text-slate-200">
                  Scan Emails
                </span>{" "}
                to load classified inbox results.
              </div>
            ) : (
                ...
                return (
                  ...
                  >
                    <div className="flex flex-col gap-3 md:flex-row md:items-start md:justify-between">
                      ...
                      <div
                        className={`inline-flex rounded-full border px-3 py-1 text-xs font-semibold uppercase tracking-[0.2em] ${typeTone(type)}`}
                      >
                        {type}
                      </div>
                    </div>
                    ...
                    <p className="mt-4 text-sm leading-7 text-slate-400">
                      {email.preview ||
                        email.snippet ||
                        email.body ||
                        "No email preview returned by the backend."}
                    </p>
          ...
          <div className="space-y-3">
            {payload.events.length === 0 ? (
              <div className="rounded-2xl border border-dashed border-white/10 px-4 py-5 text-sm text-slate-400">
                Upcoming events will appear here after the scan returns calendar
                data.
              </div>
            ) : (
              payload.events.map((event, index) => (
                <div
                  key={`${event.title || event.summary || "event"}-${index}`}
                ...
                >
                  ...
                  <div className="mt-3 text-xs uppercase tracking-[0.2em] text-cyan-400">
                    {event.source || "calendar sync"}
                  </div>
                </div>
              ))
            )}
