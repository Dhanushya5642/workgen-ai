...
def scan_emails(payload: EmailRequest):
    ...
    # Handle explicit actions (for backward compatibility)
    ...
    for message in messages:
        ...
        # AUTO-ADD to Calendar and Notion — no user permission prompts
        auto_actions = []
        start = email_info.get("start")
        if start:
            start_time = datetime.fromisoformat(start)
            title = email_info.get("title") or email_info.get("subject") or "AgentX Event"
            intent_type = email_info.get("detected_type") or "task"
            duration_minutes = int(email_info.get("duration_minutes") or 60)

            try:
                backend_main.create_calendar_event(title, start_time, intent_type, duration_minutes)
                auto_actions.append("calendar")
            except Exception as ce:
                print(f"Calendar auto-add failed: {ce}")

            try:
                backend_main.add_to_notion(title, start_time)
                auto_actions.append("notion")
            except Exception as ne:
                print(f"Notion auto-add failed: {ne}")

        email_info["auto_added"] = auto_actions
        detected_emails.append(email_info)
